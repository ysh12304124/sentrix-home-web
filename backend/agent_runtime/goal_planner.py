...
- 全库统计/金额/费用/桌数类问题（如'收了多少礼金''花了多少钱''一共几桌''相册多少张'）不要声明 structured_fact：
  没有可用于该统计的照片证据，检索返回的照片数不等于该统计值；这类问题无法确认时保持 requirements 为空，回答阶段如实说明。
- 拍摄时间/日期/年份声明 temporal_metadata；照片里的文字/数字/价格声明 visible_text；
  只有能由照片或工具结果直接确认的确定事实（如某张照片拍到的人数/读到某年份）才声明 structured_fact；
  只有用户明确要求找出照片时才增加 memory_asset。

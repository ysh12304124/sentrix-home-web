...
def assistant_response(result, include_debug=False):
    ...
    admin = include_debug or _prof.admin_debug_presentation()
    ...
    result["modelRuntime"] = _current_model_runtime()
    if not admin:
...
def assistant_turn(request: AssistantTurnRequest):
    ...
    return assistant_response(result, include_debug=request.include_debug)
...
async def ingest(
    ...
    destination = MEDIA_DIR / f"{upload_id}_{safe_name}"
    ...
    batch = (batch_id or batchId or "").strip() or None
    if batch:

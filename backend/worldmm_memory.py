...
class WorldMMMemory:
    ...
    def retrieve(self, question, options=None, limit=5):
        ...
        wants_visual = any(word in qlow for word in ("visual", "shown", "visible", "wear", "wearing", "color", "colour", "look", "depicted", "placing", "using", "architectural", "pattern", "dress", "shirt", "clothing", "what is"))
        ...
        for row in selected_visual:
            ...
            # Never expose a visual evidence row whose file cannot be loaded.
            if not payload:
                continue
            seen.add(key)

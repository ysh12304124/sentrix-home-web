  ...
  function renderModal() {
    ...
    if (modal.type === "event") {
      ...
      const relationTags = edgeLabels.map((label) => `<span class="graph-memory-relation-tag">${label}</span>`).join("")
        + `<span class="graph-memory-relation-tag candidate">LEADS_TO_CANDIDATE · ${stats.causal_candidates || 0}</span>`;

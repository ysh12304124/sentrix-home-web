(function () {
  ...
  function renderModal() {
    ...
    if (modal.type === "event") {
      ...
      const coverOptions = (modal.observations || []).map((observation) => `<option value="${escapeHtml(observation.asset_id)}" ${observation.asset_id === event.cover_asset_id ? "selected" : ""}>${escapeHtml(observation.asset?.file_name || observation.asset_id)}</option>`).join("");

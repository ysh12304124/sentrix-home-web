(function () {
  ...
  function peopleView() {
    ...
    const edgeSvg = graph.edges.filter((edge) => visibleIds.has(edge.subjectId) && visibleIds.has(edge.objectId)).map((edge) => {
      ...
      return `<g class="people-graph-edge ${isDim ? "dim" : ""}" data-action="select-people-graph-edge" data-subject-id="${escapeHtml(edge.subjectId)}" data-object-id="${escapeHtml(edge.objectId)}" data-predicate="${escapeHtml(edge.predicate)}" data-inverse="${escapeHtml(edge.inversePredicate)}"><line x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}" marker-end="url(#people-graph-arrow)"/><text x="${(from.x + to.x) / 2}" y="${(from.y + to.y) / 2 - 8}">${escapeHtml(label)}</text></g>`;
    }).join("");
    const nodeSvg = [...groups.family, ...groups.friends].map((person) => { const point = positions[person.id]; if (!point) return ""; const avatar = person.avatar_face_instance_id ? `<image href="/api/face-instances/${encodeURIComponent(person.avatar_face_instance_id)}/crop" x="${point.x - 28}" y="${point.y - 28}" width="56" height="56" preserveAspectRatio="xMidYMid slice" clip-path="url(#people-avatar-${escapeHtml(person.id)})"/>` : `<text class="people-graph-initial" x="${point.x}" y="${point.y + 6}">${escapeHtml((person.display_name || "?").slice(0, 1))}</text>`; return `<g class="people-graph-node ${person.id === state.peopleGraphSelection ? "selected" : ""} ${dim(person.id)}" data-action="select-people-graph-person" data-person-id="${escapeHtml(person.id)}" tabindex="0" aria-label="查看${escapeHtml(person.display_name)}"><clipPath id="people-avatar-${escapeHtml(person.id)}"><circle cx="${point.x}" cy="${point.y}" r="28"/></clipPath><circle cx="${point.x}" cy="${point.y}" r="34"/>${avatar}<text class="people-graph-name" x="${point.x}" y="${point.y + 52}">${escapeHtml(person.display_name || "待命名人物")}</text><circle class="people-graph-handle" cx="${point.x + 37}" cy="${point.y - 25}" r="8" data-action="start-people-graph-edge" data-person-id="${escapeHtml(person.id)}" aria-label="从${escapeHtml(person.display_name)}添加关系">+</circle></g>`; }).join("");
    ...
    const media = (selected?.representative_media || []).map((item) => `<button class="people-media-thumb" data-action="open-asset" data-asset-id="${escapeHtml(item.id)}"><img src="/api/assets/${encodeURIComponent(item.id)}/file" alt="${escapeHtml(selected.display_name)}的代表媒体" loading="lazy"/></button>`).join("");

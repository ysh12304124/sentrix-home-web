(function () {
  ...
  function performanceView() {
    ...
    const comparisonRows = (comparison.runs || []).slice(0, 8).map((run) => { const cfg = run.config_json || {}; const metrics = run.metrics_json || {}; return `<tr><td>${escapeHtml(cfg.merge_threshold == null ? "—" : Number(cfg.merge_threshold).toFixed(2))}</td><td>${escapeHtml(run.run_id || "")}</td><td>${metrics.event_count ?? "—"}</td><td>${metrics.singleton_rate == null ? "—" : `${(Number(metrics.singleton_rate) * 100).toFixed(1)}%`}</td><td>${performanceValueMs(Number(metrics.eventagg_wall_seconds || 0) * 1000)}</td><td>${run.status === "completed" ? "完成" : escapeHtml(run.status || "—")}</td></tr>`; }).join("");
    const methodComparison = comparison.runs?.length ? `<section class="content-section performance-section"><div class="section-head"><div><p class="section-kicker">视频记忆方法 A/B</p><h2>Baseline 与 EventAgg v2.1</h2></div><span class="result-count">当前：${escapeHtml(comparison.selected_method || "baseline")}</span></div><p class="performance-note">Baseline 保留原有视频场景；EventAgg 复用已有 WebP/YOLO/Pose/metadata，用 DINOv2 和时间连续性做事件级聚合，所有证据帧仍保留。</p><div class="performance-table-wrap"><table class="performance-table"><thead><tr><th>阈值</th><th>run</th><th>事件数</th><th>单帧事件</th><th>聚合耗时</th><th>状态</th></tr></thead><tbody>${comparisonRows}</tbody></table></div></section>` : "";

(function () {
  ...
  function assetCard(asset) {
    return `<button class="asset-card" data-action="open-asset" data-asset-id="${escapeHtml(asset.id)}">${assetThumb(asset, true)}<div class="asset-info"><strong>原始${escapeHtml(mediaLabel(asset.media_type))}证据</strong><small>${formatDateTime(asset.created_at)} · ${escapeHtml(mediaLabel(asset.media_type))}</small><span>${escapeHtml(assetStatusLabel(asset.status))}</span></div></button>`;
  }

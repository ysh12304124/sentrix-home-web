(function () {
  ...
  async function handleAction(action, element) {
    ...
    if (action === "open-qa-dashboard") {
      ...
      let benchPort = 8771;
      try {
        const response = await fetch("/api/photobench/ensure", { method: "POST" });
        const status = await response.json();
        benchPort = Number(status.port || benchPort);
      } catch (_) { /* 评测服务不可达时仍打开，让用户看到连接状态 */ }
      const benchUrl = `${window.location.protocol}//${window.location.hostname}:${benchPort}/`;

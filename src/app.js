(function () {
  ...
  function knowledgeView() {
    ...
    const clusterCard = (cluster) => `<article class="cluster-card"><div class="cluster-head"><div><span class="section-kicker">FACE CLUSTER</span><strong>${escapeHtml(cluster.id)}</strong></div><span class="needs-label">${cluster.member_count || cluster.samples?.length || 0} 张样本</span></div><div class="cluster-samples">${(cluster.samples || []).slice(0, 6).map((sample) => `<button data-action="open-asset" data-asset-id="${escapeHtml(sample.asset_id)}">${faceAvatar(sample.id, "人脸样本")}</button>`).join("")}</div><p>聚类置信度 ${Math.round((cluster.confidence || 0) * 100)}% · 未绑定身份</p><div class="person-actions"><button class="button small primary" data-action="confirm-cluster" data-cluster-id="${escapeHtml(cluster.id)}">确认实体</button><button class="button small ghost" data-action="reject-cluster" data-cluster-id="${escapeHtml(cluster.id)}">驳回簇</button></div></article>`;

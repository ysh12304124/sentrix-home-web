(function () {
  ...
  window.sentrixApi = {
    ...
    graphMemoryStats: (scopeId = "home-default") => request(`/api/graph-memory/stats?scope_id=${encodeURIComponent(scopeId || "home-default")}`),
    graphMemoryQuery: (payload) => request("/api/graph-memory/query", { method: "POST", body: JSON.stringify(payload) }),
    graphMemoryNode: (nodeId, scopeId = "home-default") => request(`/api/graph-memory/nodes/${encodeURIComponent(nodeId)}?scope_id=${encodeURIComponent(scopeId || "home-default")}`),
    graphMemorySubgraph: (nodeId, scopeId = "home-default", options = {}) => request(`/api/graph-memory/subgraph?${new URLSearchParams({ node_id: nodeId, scope_id: scopeId || "home-default", max_depth: options.max_depth || 2, max_nodes: options.max_nodes || 100 }).toString()}`),
    ...
    renameConversation: (id, title) => request(`/api/conversations/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ title }) }),
    deleteConversation: (id) => request(`/api/conversations/${encodeURIComponent(id)}`, { method: "DELETE" }),
    createPhotoThread: (payload) => request("/api/photo-threads", { method: "POST", body: JSON.stringify(payload) }),
    photoThreadTurnAsync: (threadId, message, viewerId = "owner") => request(`/api/photo-threads/${encodeURIComponent(threadId)}/turn`, { method: "POST", body: JSON.stringify({ message, viewer_id: viewerId }) }),
    photoThreadMessages: (threadId, limit = 20) => request(`/api/photo-threads/${encodeURIComponent(threadId)}/messages?limit=${limit}`),
    deletePhotoThread: (threadId) => request(`/api/photo-threads/${encodeURIComponent(threadId)}`, { method: "DELETE" }),

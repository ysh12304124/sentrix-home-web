...
def _normalize_preview_handle(arguments: dict, preview_handles: list[str] | None) -> tuple[dict, str | None]:
    ...
    if not requested or requested in preview_handles:
        return arguments, None
...
def record_agent2_tool_evidence(task_state, evidence_ledger, spec, *,
    ...
    generic_value = (observation.get("value") or observation.get("matches") or
                     observation.get("cards") or observation.get("events") or
                     observation.get("items"))
    # A natural failure summary is not evidence for OCR/visual tools.  Only
    # structured/actual observations may satisfy those requirement types.
    if spec.name not in {"inspect_photo", "read_photo_text"} and generic_value is None:
        generic_value = observation.get("summary")
...
class AgentRuntime:
    ...
    def run(self, message: str, *, history: str = "", task_state: dict | None = None,
        ...
        max_auto_resolution_attempts = 1
        ...
        def visual_gate_requested() -> bool:
            """Use Agent 2's visual requirement when available; otherwise regex fallback."""
            ...
            if action.get("action") == "final":
                ...
                if resolution:
                    if resolution_retries >= max_resolution_retries:
                        ...
                        auto_handle = _next_resolution_handle(task, resolution["tool"])
                        if (auto_spec is not None and auto_handle
                                and auto_resolution_attempts < max_auto_resolution_attempts
                                and turn.budget.can_tool_call()):
                            auto_resolution_attempts += 1
                            tool_name = resolution["tool"]
                            auto_args = {"asset_handle": auto_handle}
                            if tool_name == "read_photo_text":
                                auto_args["question"] = message
                            self._emit_progress(
                                turn, progress_callback, stage="inspecting", status="running",
                                text=f"正在复核照片 {auto_handle}…" if tool_name == "inspect_photo"
                                else "正在读取照片中的文字…")

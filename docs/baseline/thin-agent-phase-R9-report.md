...
- **单元/集成**：`backend.tests` **493 pass / 1 skip**（基线 448 → +45）。
...
## 5. 关闭决策（R9-7，待 153 数据）

按失败归因选择：
- **A**：Router + Hidden + latency 通过 → 关闭 Retrieval 基础阶段，进入高级 Agent/Core Memory。
- **B**：主要失败来自 Formation → 进入 Formation F1。
- **C**：主要失败来自 Embedding/Ranking → 继续 Retrieval R10。
- **D**：主要失败来自基础设施（GPU/bge 环境）→ 先做 Infrastructure Phase。

## 6. 用户输入清单（§11）

1. **153 部署 go-ahead**（生产服务重启，走 git patch/bundle + 备份 + 测试绿后）。
2. **GPU driver mismatch 修复窗口**（最终性能验收前置，D10）。
3. **bge-m3 `.venv-text`** 依赖安装批准（或指定替代宿主）。
4. **Hidden 评分**：用户持 16 GT 离线跑 `score_hidden.py`。
5. **Parser 中档模型**（可选 7B 对照）仅在需要区分"模型质量"vs"槽位方法"时拉取。

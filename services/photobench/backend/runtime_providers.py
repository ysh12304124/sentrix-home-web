...
class OpenAICompatibleInferenceProvider(InferenceProvider):
    ...
    def list_models(self) -> dict:
        if self.request_fn is not None:
            body = self.request_fn(f"{self.base_url}/models", timeout=min(15, self.timeout)) or {}
            models = [
...
class ManagerLifecycleProvider(LifecycleProvider):
    def __init__(self, manager_url: str, *, request_fn: Callable | None = None):
    ...
    def _request(self, path: str, *, payload=None, method="GET", timeout=30):
        if self.request_fn is not None:
            return self.request_fn(
                f"{self.base_url}{path}", payload=payload, method=method, timeout=timeout,
            )

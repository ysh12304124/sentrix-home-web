"""GPU smoke test for the E2B model loader.

Gate: SENTRIX_E2B_SMOKE=1 must be set, otherwise all tests are skipped.
This test requires a real GPU with the base model and LoRA adapter present.
"""

import os
import unittest


@unittest.skipUnless(
    os.getenv("SENTRIX_E2B_SMOKE") in {"1", "true", "yes", "on"},
    "SENTRIX_E2B_SMOKE=1 is required for GPU smoke tests",
)
class E2BModelSmokeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        from services.e2b_server.model import E2BModel
        base = os.getenv("E2B_BASE_MODEL", "/home/asus/models/gemma-4-E2B-it")
        adapter = os.getenv("E2B_ADAPTER", "/home/asus/models/gemma-4-e2b-lora-v2/artifacts/lora/V2_student_step47")
        cls.model = E2BModel(base, adapter)

    def test_load_and_generate_text(self):
        import asyncio
        async def run():
            await self.model.ensure_loaded()
            self.assertTrue(self.model.is_loaded)
            text = await self.model.generate("你好，请说中文。")
            self.assertIsInstance(text, str)
            self.assertGreater(len(text), 0)
        asyncio.run(run())

    def test_load_and_generate_with_image(self):
        import asyncio
        import base64
        async def run():
            await self.model.ensure_loaded()
            # A tiny 1x1 white JPEG
            tiny_jpg = base64.b64encode(
                b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00\xff\xdb\x00C"
            ).decode("ascii")
            text = await self.model.generate("描述图片", [tiny_jpg])
            self.assertIsInstance(text, str)
            self.assertGreater(len(text), 0)
        asyncio.run(run())

    @classmethod
    def tearDownClass(cls):
        import asyncio
        asyncio.run(cls.model.unload())


if __name__ == "__main__":
    unittest.main()

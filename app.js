(function () {
  ...
  function eventViewModel(event, index = 0) {
    const people = eventPeople(event).map((person) => typeof person === "string" ? person : person.name || person.display_name).filter(Boolean);
    return {
      ...event,
  ...
  function searchView() {
    ...
    return `${pageHeader("记忆搜索 / Agent", "问一句，找到一段有证据的记忆。", "答案只来自本地事件、事实、Observation 和 原生向量索引；每条证据都可以打开原始资料。")}${searchBar("例如：哪张图片里有冰箱？")}${state.searchLoading ? `<section class="empty-search"><div class="empty-symbol">◌</div><h2>正在检索本地记忆</h2><p>正在召回事件、事实、原始观察和语义上下文。</p></section>` : result ? `<section class="search-layout"><div><div class="answer-card"><div class="answer-meta"><span class="status-dot"></span>Sentrix 已完成本地检索 <span class="confidence">置信度 ${Math.round((result.confidence || 0) * 100)}% · ${escapeHtml(result.model || "本地 Agent")}</span></div><h2>检索结果</h2><p>${escapeHtml(result.answer || "证据不足，无法回答。").replace(/\n/g, "<br />")}</p><div class="answer-tags">${memoryPill("episodic", "事件证据")}${memoryPill("semantic", "事实证据")}${memoryPill("visual", "原始资料")}</div></div><div class="section-head results-head"><div><p class="section-kicker">原始证据</p><h2>回答依据的全部记录</h2></div><span class="result-count">${(result.evidence || []).length} 项</span></div><div class="evidence-list">${result.evidence?.length ? result.evidence.map(evidenceCard).join("") : emptyState("没有召回证据", "当前问题没有匹配到本地记录，Agent 没有使用示例数据。")}</div></div><aside class="trace-panel"><div class="panel-title"><span>RETRIEVAL TRACE</span><span class="live-label"><i></i>本地</span></div><h3>这次回答经过了什么？</h3>${(result.retrievalTrace || []).map((item, i) => `<div class="trace-step"><span>${String(i + 1).padStart(2, "0")}</span><div><strong>${escapeHtml(item)}</strong><small>已完成检索阶段</small></div><b>✓</b></div>`).join("")}<div class="trace-note">视频编码记忆 <span>接口预留</span></div></aside></section>` : `<section class="empty-search"><div class="empty-symbol">⌕</div><h2>从一个线索开始</h2><p>输入后会返回真实答案和原始证据，不会显示预填充结果。</p><div class="suggestions"><button data-query="图片里有什么？">图片里有什么？</button><button data-query="最近发生了什么？">最近发生了什么？</button><button data-query="哪些事实等待确认？">哪些事实等待确认？</button></div></section>`}`;
  }
  ...
  function knowledgeView() {
    ...
    const entityCard = (entity) => `<button class="entity-card" data-action="open-entity" data-entity-id="${escapeHtml(entity.id)}"><div class="entity-card-head"><span class="avatar person-avatar ${entity.status === "confirmed" ? "green" : "gray"}">${escapeHtml((entity.canonical_name || "?").slice(0, 1))}</span><span class="needs-label">${escapeHtml(entity.status)}</span></div><h2>${escapeHtml(entity.canonical_name)}</h2><p>${escapeHtml(entity.family_role || "未确认家庭角色")}</p><div class="entity-stats"><span><strong>${entity.mention_count || 0}</strong> 次出现</span><span><strong>${entity.cluster_count || 0}</strong> 个人物簇</span><span><strong>${entity.relationship_count || 0}</strong> 条关系</span></div></button>`;
    const clusterCard = (cluster) => `<article class="cluster-card"><div class="cluster-head"><div><span class="section-kicker">FACE CLUSTER</span><strong>${escapeHtml(cluster.id)}</strong></div><span class="needs-label">${cluster.member_count || cluster.samples?.length || 0} 张样本</span></div><div class="cluster-samples">${(cluster.samples || []).slice(0, 6).map((sample) => `<button data-action="open-asset" data-asset-id="${escapeHtml(sample.asset_id)}">${assetThumb({ id: sample.asset_id, file_name: sample.file_name, media_type: sample.media_type }, true)}</button>`).join("")}</div><p>聚类置信度 ${Math.round((cluster.confidence || 0) * 100)}% · 未绑定身份</p><div class="person-actions"><button class="button small primary" data-action="confirm-cluster" data-cluster-id="${escapeHtml(cluster.id)}">确认实体</button><button class="button small ghost" data-action="reject-cluster" data-cluster-id="${escapeHtml(cluster.id)}">驳回簇</button></div></article>`;
  ...
  function renderView() {
    ...
    const views = { overview, search: searchView, timeline: timelineView, people: peopleView, knowledge: knowledgeView, library: libraryView, stories: storiesView, imports: importsView, settings: settingsView };
  ...
  function renderModal() {
    ...
    if (modal.type === "event") {
      ...
      body = `<form id="modal-form"><div class="modal-kicker">FACE CLUSTER · ${escapeHtml(cluster.id)}</div><h2>确认这个人物实体</h2><p class="modal-lead">这组样本由 153 上的 buffalo_l embedding 聚类得到。确认后，所有样本会统一绑定到同一个实体。</p><div class="cluster-samples modal-samples">${(cluster.samples || []).map((sample) => `<button type="button" data-action="open-asset" data-asset-id="${escapeHtml(sample.asset_id)}">${assetThumb({ id: sample.asset_id, file_name: sample.file_name, media_type: sample.media_type }, true)}</button>`).join("")}</div><label>姓名或称呼<input name="name" placeholder="例如：妈妈" required /></label><label>家庭角色<select name="family_role"><option value="">暂不确认</option><option>母亲</option><option>父亲</option><option>孩子</option><option>祖父母</option><option>其他家庭成员</option></select></label><label>与已确认实体的关系（可选）<select name="relation_target"><option value="">暂不建立关系</option>${state.entities.filter((entity) => entity.status === "confirmed").map((entity) => `<option value="${escapeHtml(entity.id)}">${escapeHtml(entity.canonical_name)}</option>`).join("")}</select></label><label>关系类型（可选）<input name="relation_predicate" placeholder="例如：母亲、父亲、兄弟姐妹" /></label><div class="modal-actions"><button type="button" class="button ghost" data-action="close-modal">取消</button><button type="submit" class="button primary">确认并更新记忆</button></div></form>`;
    } else if (modal.type === "entity") {
      const detail = modal.detail;
      const entity = detail.entity;
      body = `<div class="modal-kicker">ENTITY · ${escapeHtml(entity.id)}</div><h2>${escapeHtml(entity.canonical_name)}</h2><p class="modal-lead">${escapeHtml(entity.summary || "这是跨多个事件维护的实体，不是单张图片的描述。")} · 状态 ${escapeHtml(entity.status)}</p><div class="detail-facts"><span>类型 · ${escapeHtml(entity.entity_type)}</span><span>角色 · ${escapeHtml(entity.family_role || "未确认")}</span><span>人物簇 · ${detail.clusters.length}</span></div><div class="section-head"><div><p class="section-kicker">关系与事实</p><h3>${detail.relationships.length} 条关系 · ${detail.facts.length} 条事实</h3></div></div><div class="evidence-list">${detail.relationships.concat(detail.facts.map((fact) => ({ ...fact, subject_name: fact.subject, object_name: fact.object, predicate: fact.predicate, status: fact.status }))).map((item) => `<div class="fact-review-row"><div><strong>${escapeHtml(item.subject_name || item.subject)} ${escapeHtml(item.predicate)} ${escapeHtml(item.object_name || item.object)}</strong><small>${escapeHtml(item.status)} · 证据 ${(item.evidence_ids_json || []).join(", ")}</small></div></div>`).join("") || emptyState("暂无关系和事实", "确认新的实体关系后，长期语义记忆会显示在这里。")}</div><div class="section-head"><div><p class="section-kicker">人物簇样本</p><h3>原始证据</h3></div></div><div class="cluster-grid">${detail.clusters.map((cluster) => `<div class="cluster-card"><div class="cluster-head"><strong>${escapeHtml(cluster.id)}</strong><span>${cluster.member_count} 张</span></div><div class="cluster-samples">${(cluster.samples || []).map((sample) => `<button data-action="open-asset" data-asset-id="${escapeHtml(sample.asset_id)}">${assetThumb({ id: sample.asset_id, file_name: sample.file_name, media_type: sample.media_type }, true)}</button>`).join("")}</div></div>`).join("")}</div>`;
  ...
  async function refreshData() {
    ...
    const calls = await Promise.allSettled([
      window.sentrixApi.dashboard(), window.sentrixApi.events(), window.sentrixApi.assets("?limit=1000"), window.sentrixApi.persons(), window.sentrixApi.stories(), window.sentrixApi.health(), window.sentrixApi.entities(), window.sentrixApi.faceClusters(), window.sentrixApi.relationships(),
    ]);
    ...
    state.persons = calls[3].status === "fulfilled" ? calls[3].value.persons || [] : [];
